...
class TestCreateCommentEndpoint(TestCase):
    ...
    def setUp(self):
        ...
        self.create_comment_url = reverse(
            'comments:comments',
            kwargs={'author_id': self.author.id, 'post_id': self.post.id}
        )
    ...
    def test_create_comment_endpoint(self):
        ...
        res = self.client.post(self.create_comment_url, PAYLOAD)
        comment = res.data
    ...
    def test_create_comment_endpoint_for_friends(self):
        ...
        res = self.client2.post(self.create_comment_url, PAYLOAD)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)

        # second friend makes a comment
        self.client3 = APIClient()
        self.client3.force_authenticate(user=self.author3)
        res = self.client3.post(self.create_comment_url, PAYLOAD)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)

        # original author can see all three comments
        res = self.client2.get(self.create_comment_url)
